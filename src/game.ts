...
drawBtn?.addEventListener("click", () => {
  ...
  const lines = `
      ...
      <pre class="result-line">Ergebnis&#9&#9${n}</pre>
      <pre class="result-line">Bereich&#9&#9${actualRange}</pre>
      <pre class="result-line">Dein Tipp&#9${guessText}</pre>
      <pre class="result-line">Deine Superzahl&#9&#9${s}</pre>
    `;
